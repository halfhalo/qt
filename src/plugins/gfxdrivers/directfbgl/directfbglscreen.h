...
#ifndef DIRECTFBSCREEN_H
#define DIRECTFBSCREEN_H
...
class DirectFBGLScreenPrivate;
class DirectFBGLScreen : public QGLScreen
{
public:
    DirectFBGLScreen(int display_id);
    ~DirectFBGLScreen();
...
#endif
private:
    DirectFBGLScreenPrivate *d_ptr;
};

class DirectFBGLScreenSurfaceFunctions : public QGLScreenSurfaceFunctions
{
public:
    DirectFBGLScreenSurfaceFunctions(DirectFBGLScreen *s, int screenNum)
...
Q_DECLARE_OPERATORS_FOR_FLAGS(DirectFBGLScreen::SurfaceCreationOptions);
Q_DECLARE_OPERATORS_FOR_FLAGS(DirectFBGLScreen::DirectFBFlags);

inline bool DirectFBGLScreen::isPremultiplied(QImage::Format format)
{
    switch (format) {
...
inline bool DirectFBGLScreen::hasAlphaChannel(DFBSurfacePixelFormat format)
{
    switch (format) {
    case DSPF_ARGB1555:
    case DSPF_ARGB:
...
inline bool DirectFBGLScreen::hasAlphaChannel(IDirectFBSurface *surface)
{
    Q_ASSERT(surface);
    DFBSurfacePixelFormat format;
    surface->GetPixelFormat(surface, &format);
    return DirectFBGLScreen::hasAlphaChannel(format);
}

QT_END_NAMESPACE

QT_END_HEADER
#endif // DIRECTFBSCREEN_H
