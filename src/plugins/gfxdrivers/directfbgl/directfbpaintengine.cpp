...
class SurfaceCache;
class DirectFBPaintEnginePrivate : public QRasterPaintEnginePrivate
{
public:
    ...
    inline static uint getTransformationType(const QTransform &transform)
    {
        ...
        if (transform.m11() < 0)
            ret |= DirectFBPaintEnginePrivate::Matrix_NegativeScaleX;
        if (transform.m22() < 0)
            ret |= DirectFBPaintEnginePrivate::Matrix_NegativeScaleY;
        return ret;
    }
    ...
    DirectFBPaintEnginePrivate(DirectFBPaintEngine *p);
    ~DirectFBPaintEnginePrivate();
    ...
    void lock();
    void unlock();
    static inline void unlock(DirectFBPaintDevice *device);
...
struct CachedImage
{
    ...
    ~CachedImage()
    {
        if (surface && DirectFBGLScreen::instance()) {
            DirectFBGLScreen::instance()->releaseDFBSurface(surface);
        }
    }
};
...
static inline uint rasterFallbacksMask(PaintOperation op)
{
    ...
    if (warningMask < 0) {
        ...
        if (!warning.isEmpty()) {
            qWarning("DirectFBPaintEngine QT_DIRECTFB_WARN_ON_RASTERFALLBACKS Unknown operation(s): %s",
                     qPrintable(warning.join(QLatin1String("|"))));
        }
        if (!disable.isEmpty()) {
            qWarning("DirectFBPaintEngine QT_DIRECTFB_DISABLE_RASTERFALLBACKS Unknown operation(s): %s",
...
static void rasterFallbackWarn(const char *msg, const char *func, const device *dev,
                               DirectFBPaintEnginePrivate *priv,
...
#define CLIPPED_PAINT(operation) {                                      \
        ...
        switch (d->clipType) {                                          \
        case DirectFBPaintEnginePrivate::NoClip:                       \
        case DirectFBPaintEnginePrivate::RectClip:                     \
            operation;                                                  \
            break;                                                      \
        case DirectFBPaintEnginePrivate::RegionClip: {                 \
            Q_ASSERT(d->clip());                                        \
            ...
            d->updateClip();                                            \
            break; }                                                    \
        case DirectFBPaintEnginePrivate::ComplexClip:                  \
        case DirectFBPaintEnginePrivate::ClipUnset:                    \
...
DirectFBPaintEngine::DirectFBPaintEngine(QPaintDevice *device)
    : QRasterPaintEngine(*(new DirectFBPaintEnginePrivate(this)), device)
{
}

DirectFBPaintEngine::~DirectFBPaintEngine()
{
}

bool DirectFBPaintEngine::begin(QPaintDevice *device)
{
    Q_D(DirectFBPaintEngine);
    if (device->devType() == QInternal::CustomRaster) {
        d->dfbDevice = static_cast<DirectFBPaintDevice*>(device);
    } else if (device->devType() == QInternal::Pixmap) {
        QPixmapData *data = static_cast<QPixmap*>(device)->pixmapData();
        Q_ASSERT(data->classId() == QPixmapData::DirectFBClass);
        DirectFBPixmapData *dfbPixmapData = static_cast<DirectFBPixmapData*>(data);
        DirectFBPaintEnginePrivate::unlock(dfbPixmapData);
        d->dfbDevice = static_cast<DirectFBPaintDevice*>(dfbPixmapData);
    }
    ...
    if (!d->surface) {
        qFatal("DirectFBPaintEngine used on an invalid device: 0x%x",
               device->devType());
    }
    d->isPremultiplied = DirectFBGLScreen::isPremultiplied(d->dfbDevice->format());
...
void DirectFBPaintEngine::clipEnabledChanged()
{
    Q_D(DirectFBPaintEngine);
...
void DirectFBPaintEngine::brushChanged()
{
    Q_D(DirectFBPaintEngine);
...
void DirectFBPaintEngine::penChanged()
{
    Q_D(DirectFBPaintEngine);
    d->setPen(state()->pen);
...
void DirectFBPaintEngine::opacityChanged()
{
    Q_D(DirectFBPaintEngine);
...
void DirectFBPaintEngine::compositionModeChanged()
{
    Q_D(DirectFBPaintEngine);
...
void DirectFBPaintEngine::renderHintsChanged()
{
    Q_D(DirectFBPaintEngine);
...
void DirectFBPaintEngine::transformChanged()
{
    Q_D(DirectFBPaintEngine);
...
void DirectFBPaintEngine::setState(QPainterState *state)
{
    Q_D(DirectFBPaintEngine);
...
void DirectFBPaintEngine::clip(const QVectorPath &path, Qt::ClipOperation op)
{
    Q_D(DirectFBPaintEngine);
...
void DirectFBPaintEngine::clip(const QRegion &region, Qt::ClipOperation op)
{
    Q_D(DirectFBPaintEngine);
    const bool wasInClip = d->inClip;
    d->inClip = true;
...
void DirectFBPaintEngine::clip(const QRect &rect, Qt::ClipOperation op)
{
    Q_D(DirectFBPaintEngine);
    const bool wasInClip = d->inClip;
    d->inClip = true;
...
void DirectFBPaintEngine::drawRects(const QRect *rects, int rectCount)
{
    Q_D(DirectFBPaintEngine);
    ...
    if ((d->transformationType & DirectFBPaintEnginePrivate::Matrix_RectsUnsupported)
        || !d->supportedPen
        || d->clipType == DirectFBPaintEnginePrivate::ComplexClip
        || !d->supportedBrush
...
void DirectFBPaintEngine::drawRects(const QRectF *rects, int rectCount)
{
    Q_D(DirectFBPaintEngine);
    const QPen &pen = state()->pen;
    const QBrush &brush = state()->brush;
...
void DirectFBPaintEngine::drawLines(const QLine *lines, int lineCount)
{
    Q_D(DirectFBPaintEngine);
...
void DirectFBPaintEngine::drawLines(const QLineF *lines, int lineCount)
{
    Q_D(DirectFBPaintEngine);
...
void DirectFBPaintEngine::drawImage(const QRectF &r, const QImage &image,
                                     const QRectF &sr,
                                     Qt::ImageConversionFlags flags)
{
    ...
    */
    ...
    if (!d->supportedComposition
        || (d->transformationType & DirectFBPaintEnginePrivate::Matrix_BlitsUnsupported)
        || (d->clipType == DirectFBPaintEnginePrivate::ComplexClip)
        || (!d->supportsStretchBlit() && state()->matrix.mapRect(r).size() != sr.size())
#ifndef QT_DIRECTFB_IMAGECACHE
        || (DirectFBGLScreen::getSurfacePixelFormat(image.format()) == DSPF_UNKNOWN)
#elif defined QT_NO_DIRECTFB_PREALLOCATED
        || (DirectFBPaintEnginePrivate::cacheCost(image) > imageCache.maxCost())
#endif
        )
#endif
    {
    ...
    if (image.hasAlphaChannel())
        blitFlags |= DirectFBPaintEnginePrivate::HasAlpha;
    if (DirectFBGLScreen::isPremultiplied(image.format()))
        blitFlags |= DirectFBPaintEnginePrivate::Premultiplied;
...
void DirectFBPaintEngine::drawImage(const QPointF &p, const QImage &img)
{
    drawImage(QRectF(p, img.size()), img, img.rect());
}

void DirectFBPaintEngine::drawPixmap(const QRectF &r, const QPixmap &pixmap,
                                      const QRectF &sr)
{
    Q_D(DirectFBPaintEngine);
    ...
    if (pixmap.pixmapData()->classId() != QPixmapData::DirectFBClass) {
        ...
        Q_ASSERT(data->classId() == QPixmapData::DirectFBClass);
        DirectFBPixmapData *dfbData = static_cast<DirectFBPixmapData*>(data);
        if (!d->supportedComposition
            || (d->transformationType & DirectFBPaintEnginePrivate::Matrix_BlitsUnsupported)
            || (d->clipType == DirectFBPaintEnginePrivate::ComplexClip)
            || (!d->supportsStretchBlit() && state()->matrix.mapRect(r).size() != sr.size())) {
            ...
            DirectFBPaintEnginePrivate::unlock(dfbData);
            IDirectFBSurface *s = dfbData->directFBSurface();
            uint blitFlags = 0;
            if (pixmap.hasAlphaChannel())
                blitFlags |= DirectFBPaintEnginePrivate::HasAlpha;
            if (DirectFBGLScreen::isPremultiplied(dfbData->pixelFormat()))
                blitFlags |= DirectFBPaintEnginePrivate::Premultiplied;
...
void DirectFBPaintEngine::drawPixmap(const QPointF &p, const QPixmap &pm)
{
    drawPixmap(QRectF(p, pm.size()), pm, pm.rect());
}

void DirectFBPaintEngine::drawTiledPixmap(const QRectF &r,
                                           ...
                                           const QPointF &offset)
{
    ...
    if (pixmap.pixmapData()->classId() != QPixmapData::DirectFBClass) {
        ...
        DirectFBPixmapData *dfbData = static_cast<DirectFBPixmapData*>(pixmapData);
        const QImage *img = dfbData->buffer();
        d->lock();
...
void DirectFBPaintEngine::stroke(const QVectorPath &path, const QPen &pen)
{
...
void DirectFBPaintEngine::drawPath(const QPainterPath &path)
{
...
void DirectFBPaintEngine::drawPoints(const QPointF *points, int pointCount)
{
    Q_D(DirectFBPaintEngine);
...
void DirectFBPaintEngine::drawPoints(const QPoint *points, int pointCount)
{
    Q_D(DirectFBPaintEngine);
...
void DirectFBPaintEngine::drawEllipse(const QRectF &rect)
{
...
void DirectFBPaintEngine::drawPolygon(const QPointF *points, int pointCount,
...
void DirectFBPaintEngine::drawPolygon(const QPoint *points, int pointCount,
                                       PolygonDrawMode mode)
{
    RASTERFALLBACK(DRAW_POLYGON, pointCount, mode, VOID_ARG());
    Q_D(DirectFBPaintEngine);
...
void DirectFBPaintEngine::drawTextItem(const QPointF &p,
...
void DirectFBPaintEngine::fill(const QVectorPath &path, const QBrush &brush)
{
...
void DirectFBPaintEngine::drawRoundedRect(const QRectF &rect, qreal xrad, qreal yrad, Qt::SizeMode mode)
{
...
void DirectFBPaintEngine::drawStaticTextItem(QStaticTextItem *item)
{
...
void DirectFBPaintEngine::fillRect(const QRectF &rect, const QBrush &brush)
{
    Q_D(DirectFBPaintEngine);
    if (brush.style() == Qt::NoBrush)
        return;
    if (d->clipType != DirectFBPaintEnginePrivate::ComplexClip) {
        switch (brush.style()) {
        ...
        case Qt::TexturePattern: {
            ...
            if (!d->supportedComposition
                || (DirectFBPaintEnginePrivate::getTransformationType(transform) & DirectFBPaintEnginePrivate::Matrix_BlitsUnsupported)
...
void DirectFBPaintEngine::fillRect(const QRectF &rect, const QColor &color)
{
    if (!color.isValid())
        return;
    Q_D(DirectFBPaintEngine);
...
void DirectFBPaintEngine::drawBufferSpan(const uint *buffer, int bufsize,
...
void DirectFBPaintEngine::initImageCache(int size)
{
    Q_ASSERT(size >= 0);
...
DirectFBPaintEnginePrivate::DirectFBPaintEnginePrivate(DirectFBPaintEngine *p)
...
{
    fb = DirectFBGLScreen::instance()->dfb();
    surfaceCache = new SurfaceCache;
}

DirectFBPaintEnginePrivate::~DirectFBPaintEnginePrivate()
{
    delete surfaceCache;
}


void DirectFBPaintEnginePrivate::lock()
{
...
void DirectFBPaintEnginePrivate::unlock()
{
    Q_ASSERT(dfbDevice);
#ifdef QT_DIRECTFB_SUBSURFACE
    dfbDevice->syncPending = true;
#else
    DirectFBPaintEnginePrivate::unlock(dfbDevice);
#endif
}

void DirectFBPaintEnginePrivate::unlock(DirectFBPaintDevice *device)
{
...
void DirectFBPaintEnginePrivate::setBrush(const QBrush &brush)
{
     supportedBrush = (brush.style() == Qt::NoBrush) || (brush.style() == Qt::SolidPattern && !antialiased);
}

void DirectFBPaintEnginePrivate::setCompositionMode(QPainter::CompositionMode mode)
{
    if (!surface)
        return;
...
void DirectFBPaintEnginePrivate::setPen(const QPen &pen)
{
...
void DirectFBPaintEnginePrivate::setTransform(const QTransform &transform)
{
    transformationType = getTransformationType(transform);
    setPen(engine->state()->pen);
}

void DirectFBPaintEnginePrivate::setRenderHints(QPainter::RenderHints hints)
{
...
void DirectFBPaintEnginePrivate::prepareForBlit(uint flags)
{
...
bool DirectFBPaintEnginePrivate::prepareForDraw(const QColor &color)
{
    Q_ASSERT(surface);
...
IDirectFBSurface *DirectFBPaintEnginePrivate::getSurface(const QImage &img, bool *release)
{
#ifdef QT_NO_DIRECTFB_IMAGECACHE
    *release = true;
    return DirectFBGLScreen::instance()->createDFBSurface(img, img.format(), DirectFBGLScreen::DontTrackSurface);
#else
    ...
    *release = false;
    ...
    const QImage::Format format = (img.format() == screen->alphaPixmapFormat() || DirectFBPixmapData::hasAlphaChannel(img)
                                   ? screen->alphaPixmapFormat() : screen->pixelFormat());


    IDirectFBSurface *surface = screen->createDFBSurface(img, format,
                                                         cache
                                                         ? DirectFBGLScreen::TrackSurface
                                                         : DirectFBGLScreen::DontTrackSurface);
    if (cache) {
...
void DirectFBPaintEnginePrivate::blit(const QRectF &dest, IDirectFBSurface *s, const QRectF &src)
{
    ...
    if (result != DFB_OK)
        DirectFBError("DirectFBPaintEngine::drawPixmap()", result);
}
...
void DirectFBPaintEnginePrivate::drawTiledPixmap(const QRectF &dest, const QPixmap &pixmap,
...
void DirectFBPaintEnginePrivate::updateClip()
{
    Q_ASSERT(surface);
    currentClip = QRect();
...
bool DirectFBPaintEnginePrivate::supportsStretchBlit() const
{
...
void DirectFBPaintEnginePrivate::systemStateChanged()
{
...
IDirectFBSurface *SurfaceCache::getSurface(const uint *buf, int size)
{
    ...
    const DFBSurfaceDescription description = DirectFBGLScreen::getSurfaceDescription(buf, size);
    surface = DirectFBGLScreen::instance()->createDFBSurface(description, DirectFBGLScreen::TrackSurface, 0);
    if (!surface)
        qWarning("DirectFBPaintEngine: SurfaceCache: Unable to create surface");
                               ...
                               const char *nameThree, const T3 &three)
{
    ...
    dbg << ((priv->transformationType & DirectFBPaintEnginePrivate::Matrix_BlitsUnsupported) ? "*" : "") << QString::fromLatin1("transformationType 0x%1").arg(priv->transformationType, 3, 16, QLatin1Char('0'));
    ...
    dbg << (priv->clipType == DirectFBPaintEnginePrivate::ComplexClip ? "*" : "") << "clipType" << priv->clipType;
