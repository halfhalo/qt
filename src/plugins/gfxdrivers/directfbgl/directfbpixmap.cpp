...
DirectFBPixmapData::DirectFBPixmapData(DirectFBGLScreen *screen, PixelType pixelType)
    : QPixmapData(pixelType, DirectFBClass), DirectFBPaintDevice(screen),
      alpha(false)
{
    setSerialNumber(0);
}

DirectFBPixmapData::~DirectFBPixmapData()
{
}

void DirectFBPixmapData::resize(int width, int height)
{
    if (width <= 0 || height <= 0) {
        invalidate();
        return;
    ...
    d = DirectFBGLScreen::depth(imageFormat);
    alpha = false;
    if (!dfbSurface) {
        invalidate();
        qWarning("DirectFBPixmapData::resize(): Unable to allocate surface");
        return;
    }
...
bool DirectFBPixmapData::hasAlphaChannel(const QImage &img, Qt::ImageConversionFlags flags)
{
    if (img.depth() == 1)
        return true;
#ifdef QT_DIRECTFB_OPAQUE_DETECTION
...
bool DirectFBPixmapData::fromFile(const QString &filename, const char *format,
...
bool DirectFBPixmapData::fromData(const uchar *buffer, uint len, const char *format,
                                   Qt::ImageConversionFlags flags)
{
    if (flags == Qt::AutoColor) {
        DFBDataBufferDescription description;
...
template <typename T> struct DirectFBInterfaceCleanupHandler
{
    static void cleanup(T *t) { if (t) t->Release(t); }
};

template <typename T>
class QDirectFBPointer : public QScopedPointer<T, DirectFBInterfaceCleanupHandler<T> >
{
public:
    QDirectFBPointer(T *t = 0)
        : QScopedPointer<T, DirectFBInterfaceCleanupHandler<T> >(t)
    {}
};

bool DirectFBPixmapData::fromDataBufferDescription(const DFBDataBufferDescription &dataBufferDescription)
{
    ...
    if ((result = dfb->CreateDataBuffer(dfb, &dataBufferDescription, &dataBufferPtr)) != DFB_OK) {
        DirectFBError("DirectFBPixmapData::fromDataBufferDescription()", result);
        return false;
    }
    ...
    if (result != DFB_OK) {
        DirectFBError("DirectFBPixmapData::fromSurfaceDescription(): Can't get image description", result);
        return false;
    }
    ...
    if ((result = provider->GetSurfaceDescription(provider.data(), &surfaceDescription)) != DFB_OK) {
        DirectFBError("DirectFBPixmapData::fromDataBufferDescription(): Can't get surface description", result);
        return false;
    }
    ...
    dfbSurface = screen->createDFBSurface(QSize(surfaceDescription.width, surfaceDescription.height),
                                          imageFormat, DirectFBGLScreen::TrackSurface);

    result = provider->RenderTo(provider.data(), dfbSurface, 0);
    if (result != DFB_OK) {
        DirectFBError("DirectFBPixmapData::fromSurfaceDescription(): Can't render to surface", result);
        return false;
    }
...
void DirectFBPixmapData::fromImage(const QImage &img,
                                    Qt::ImageConversionFlags flags)
{
    alpha = DirectFBPixmapData::hasAlphaChannel(img, flags);
    imageFormat = alpha ? screen->alphaPixmapFormat() : screen->pixelFormat();
    QImage image;
    ...
    dfbSurface = screen->createDFBSurface(image, image.format(), DirectFBGLScreen::NoPreallocated | DirectFBGLScreen::TrackSurface);
    if (!dfbSurface) {
        qWarning("DirectFBPixmapData::fromImage()");
        invalidate();
        return;
    }
...
void DirectFBPixmapData::copy(const QPixmapData *data, const QRect &rect)
{
    ...
    const DirectFBPixmapData *otherData = static_cast<const DirectFBPixmapData*>(data);
#ifdef QT_NO_DIRECTFB_SUBSURFACE
    if (otherData->lockFlags()) {
        const_cast<DirectFBPixmapData*>(otherData)->unlockSurface();
    }
#endif
    IDirectFBSurface *src = otherData->directFBSurface();
    alpha = data->hasAlphaChannel();
    imageFormat = (alpha
                   ? DirectFBGLScreen::instance()->alphaPixmapFormat()
                   : DirectFBGLScreen::instance()->pixelFormat());


    dfbSurface = screen->createDFBSurface(rect.size(), imageFormat,
                                          DirectFBGLScreen::TrackSurface);
    if (!dfbSurface) {
        qWarning("DirectFBPixmapData::copy()");
        invalidate();
        return;
    }
...
#endif
    if (result != DFB_OK) {
        DirectFBError("DirectFBPixmapData::copy()", result);
        invalidate();
        return;
    }
...
void DirectFBPixmapData::fill(const QColor &color)
{
    if (!serialNumber())
        return;
    ...
    if (alpha && isOpaqueFormat(imageFormat)) {
        ...
        d = DirectFBGLScreen::depth(imageFormat);
        dfbSurface = screen->createDFBSurface(size, screen->alphaPixmapFormat(), DirectFBGLScreen::TrackSurface);
        setSerialNumber(++global_ser_no);
        if (!dfbSurface) {
            qWarning("DirectFBPixmapData::fill()");
            invalidate();
            return;
        }
    }
...
QPixmap DirectFBPixmapData::transformed(const QTransform &transform,
                                         Qt::TransformationMode mode) const
{
    DirectFBPixmapData *that = const_cast<DirectFBPixmapData*>(this);
#ifdef QT_NO_DIRECTFB_SUBSURFACE
    if (lockFlags())
        that->unlockSurface();
#endif
        ...
        || mode != Qt::FastTransformation)
    {
        ...
        DirectFBPixmapData *data = new DirectFBPixmapData(screen, QPixmapData::PixmapType);
...
QImage DirectFBPixmapData::toImage() const
{
    if (!dfbSurface)
        return QImage();
    ...
    QImage ret(w, h, DirectFBGLScreen::getImageFormat(dfbSurface));
    if (IDirectFBSurface *imgSurface = screen->createDFBSurface(ret, DirectFBGLScreen::DontTrackSurface)) {
        if (hasAlphaChannel()) {
...
QPaintEngine *DirectFBPixmapData::paintEngine() const
{
    if (!engine) {
        // DirectFBPixmapData is also a QCustomRasterPaintDevice, so pass
        // that to the paint engine:
        DirectFBPixmapData *that = const_cast<DirectFBPixmapData*>(this);
        that->engine = new DirectFBPaintEngine(that);
    }
    return engine;
}

QImage *DirectFBPixmapData::buffer()
{
    if (!lockFlgs) {
...
bool DirectFBPixmapData::scroll(int dx, int dy, const QRect &rect)
{
    if (!dfbSurface) {
        return false;
    }
    ...
    if (result != DFB_OK) {
        DirectFBError("DirectFBPixmapData::scroll", result);
        return false;
    }
...
void DirectFBPixmapData::invalidate()
{
    if (dfbSurface) {
        screen->releaseDFBSurface(dfbSurface);
        dfbSurface = 0;
...
Q_GUI_EXPORT IDirectFBSurface *qt_directfb_surface_for_pixmap(const QPixmap &pixmap)
{
    ...
    if (!data || data->classId() != QPixmapData::DirectFBClass)
        return 0;
    const DirectFBPixmapData *dfbData = static_cast<const DirectFBPixmapData*>(data);
