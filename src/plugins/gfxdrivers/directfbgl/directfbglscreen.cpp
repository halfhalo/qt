...
#include "directfbpixmap.h"
#include "directfbmouse.h"
#include "directfbglkeyboard.h"
...
class DirectFBGLScreenPrivate : public QObject //, public QWSGraphicsSystem
{
    Q_OBJECT
public:
    DirectFBGLScreenPrivate(DirectFBGLScreen *qptr);
    ~DirectFBGLScreenPrivate();
...
public:
    ...
    DirectFBGLScreen::DirectFBFlags directFBFlags;
    ...
    DirectFBMouseHandler *mouse;
#endif
#ifndef QT_NO_DIRECTFB_KEYBOARD
    DirectFBGLKeyboardHandler *keyboard;
#endif
    ...
    DirectFBGLScreen *q;
    static DirectFBGLScreen *instance;
};

DirectFBGLScreen *DirectFBGLScreenPrivate::instance = 0;

DirectFBGLScreenPrivate::DirectFBGLScreenPrivate(DirectFBGLScreen *qptr)
    : QObject(), dfb(0), flipFlags(DSFLIP_NONE),
      directFBFlags(DirectFBGLScreen::NoFlags), alphaPixmapFormat(QImage::Format_Invalid),
      dfbScreen(0)
#ifndef QT_NO_DIRECTFB_LAYER
    , dfbLayer(0)
#endif
    ...
    , q(qptr)
{
    ...
    if(!QWSServer::instance())
        qWarning("The application is not the QWSServer!");
    else
...
DirectFBGLScreenPrivate::~DirectFBGLScreenPrivate()
{
#ifndef QT_NO_DIRECTFB_MOUSE
    delete mouse;
#endif
...
IDirectFBSurface *DirectFBGLScreen::createDFBSurface(const QImage &image, QImage::Format format, SurfaceCreationOptions options, DFBResult *resultPtr)
{
    qDebug("[DIRECTFBGL] Creating a DFBSurface for 2D rendering");
    if (image.isNull()) // assert?
        return 0;

    if (DirectFBGLScreen::getSurfacePixelFormat(format) == DSPF_UNKNOWN) {
        format = DirectFBPixmapData::hasAlphaChannel(image) ? d_ptr->alphaPixmapFormat : pixelFormat();
    }
    ...
    if (doMemCopy) {
        int bplDFB;
        uchar *mem = DirectFBGLScreen::lockSurface(surface, DSLF_WRITE, &bplDFB);
        if (mem) {
    ...
    if (image.colorCount() != 0 && surface)
        DirectFBGLScreen::setSurfaceColorTable(surface, image);
#endif
    return surface;
}

IDirectFBSurface *DirectFBGLScreen::copyDFBSurface(IDirectFBSurface *src,
                                                  ...
                                                  DFBResult *result)
{
    ...
    DFBSurfaceBlittingFlags flags = DirectFBGLScreen::hasAlphaChannel(surface)
...
IDirectFBSurface *DirectFBGLScreen::createDFBSurface(const QSize &size,
                                                    QImage::Format format,
                                                    SurfaceCreationOptions options,
                                                    DFBResult *result)
{
    ...
    if (!DirectFBGLScreen::initSurfaceDescriptionPixelFormat(&desc, format))
        return 0;
...
IDirectFBSurface *DirectFBGLScreen::createDFBSurface(DFBSurfaceDescription desc, SurfaceCreationOptions options, DFBResult *resultPtr)
{
    DFBResult tmp;
    ...
    if (!d_ptr->dfb) {
        qWarning("DirectFBGLScreen::createDFBSurface() - not connected");
        return 0;
    }
    ...
    if (d_ptr->directFBFlags & VideoOnly
        ...
        if (result != DFB_OK
            ...
            qWarning("DirectFBGLScreen::createDFBSurface() Failed to create surface in video memory!\n"
    ...
    if (result != DFB_OK) {
        qWarning("DirectFBGLScreen::createDFBSurface() Failed!\n"
                 "   Flags %0x Caps %0x width %d height %d pixelformat %0x %d preallocated %p %d\n%s",
                 desc.flags, desc.caps, desc.width, desc.height,
                 desc.pixelformat, DFB_PIXELFORMAT_INDEX(desc.pixelformat),
                 desc.preallocated[0].data, desc.preallocated[0].pitch,
...
IDirectFBSurface *DirectFBGLScreen::getSubSurface(IDirectFBSurface *surface,
                                                 const QRect &rect,
                                                 SurfaceCreationOptions options,
                                                 DFBResult *resultPtr)
{
...
void DirectFBGLScreen::releaseDFBSurface(IDirectFBSurface *surface)
{
    Q_ASSERT(DirectFBGLScreen::instance());
    Q_ASSERT(surface);
    surface->Release(surface);
    if (!d_ptr->allocatedSurfaces.remove(surface))
        qWarning("DirectFBGLScreen::releaseDFBSurface() - %p not in list", surface);

    qDebug("Released surface at %p. New count = %d", surface, d_ptr->allocatedSurfaces.count());
}

DirectFBGLScreen::DirectFBFlags DirectFBGLScreen::directFBFlags() const
{
    return d_ptr->directFBFlags;
}

IDirectFB *DirectFBGLScreen::dfb()
{
    return d_ptr->dfb;
}
...
#ifndef QT_NO_DIRECTFB_LAYER
IDirectFBDisplayLayer *DirectFBGLScreen::dfbDisplayLayer()
{
    return d_ptr->dfbLayer;
}
#endif

DFBSurfacePixelFormat DirectFBGLScreen::getSurfacePixelFormat(QImage::Format format)
{
    switch (format) {
...
QImage::Format DirectFBGLScreen::getImageFormat(IDirectFBSurface *surface)
{
...
DFBSurfaceDescription DirectFBGLScreen::getSurfaceDescription(const uint *buffer,
                                                             int length)
{
    DFBSurfaceDescription description;
    memset(&description, 0, sizeof(DFBSurfaceDescription));
...
#ifndef QT_NO_DIRECTFB_PALETTE
void DirectFBGLScreen::setSurfaceColorTable(IDirectFBSurface *surface,
                                           const QImage &image)
{
    if (!surface)
        return;
    ...
    if (result != DFB_OK) {
        DirectFBError("DirectFBGLScreen::setSurfaceColorTable GetPalette",
                      result);
        return;
    }
    result = palette->SetEntries(palette, colors.data(), numColors, 0);
    if (result != DFB_OK) {
        DirectFBError("DirectFBGLScreen::setSurfaceColorTable SetEntries",
                      result);
    }
...
class Q_GUI_EXPORT DirectFBGLScreenCursor : public QScreenCursor
{
public:
    DirectFBGLScreenCursor();
...
DirectFBGLScreenCursor::DirectFBGLScreenCursor()
{
    IDirectFB *fb = DirectFBGLScreen::instance()->dfb();
    if (!fb)
        qFatal("DirectFBGLScreenCursor: DirectFB not initialized");
    ...
    if (result != DFB_OK) {
        DirectFBError("DirectFBGLScreenCursor::hide: "
...
bool DirectFBGLScreenCursor::createWindow()
{
    Q_ASSERT(!window);
    ...
    if (result != DFB_OK) {
        DirectFBError("DirectFBGLScreenCursor::createWindow: Unable to create window", result);
        return false;
    }
    result = window->SetOpacity(window, 255);
    if (result != DFB_OK) {
        DirectFBError("DirectFBGLScreenCursor::createWindow: Unable to set opacity ", result);
        return false;
    }

    result = window->SetStackingClass(window, DWSC_UPPER);
    if (result != DFB_OK) {
        DirectFBError("DirectFBGLScreenCursor::createWindow: Unable to set stacking class ", result);
        return false;
    }

    result = window->RaiseToTop(window);
    if (result != DFB_OK) {
        DirectFBError("DirectFBGLScreenCursor::createWindow: Unable to raise window ", result);
        return false;
    }
...
void DirectFBGLScreenCursor::move(int x, int y)
{
    pos = QPoint(x, y);
#ifdef QT_DIRECTFB_WINDOW_AS_CURSOR
    if (window) {
        ...
        if (result != DFB_OK) {
            DirectFBError("DirectFBGLScreenCursor::move: Unable to move window", result);
        }
    }
#else
...
void DirectFBGLScreenCursor::hide()
{
    if (enable) {
        enable = false;
        DFBResult result;
...
void DirectFBGLScreenCursor::show()
{
    if (!enable) {
        enable = true;
        DFBResult result;
        result = layer->SetCooperativeLevel(layer, DLSCL_ADMINISTRATIVE);
        if (result != DFB_OK) {
            DirectFBError("DirectFBGLScreenCursor::show: "
                          "Unable to set cooperative level", result);
        }
...
void DirectFBGLScreenCursor::set(const QImage &image, int hotx, int hoty)
{
    DirectFBGLScreen *screen = DirectFBGLScreen::instance();
    if (!screen)
        return;
    ...
    if (image.isNull()) {
        ...
        IDirectFBSurface *surface = screen->createDFBSurface(cursor, screen->alphaPixmapFormat(),
                                                             DirectFBGLScreen::DontTrackSurface, &result);
        if (!surface) {
        ...
        if (window || createWindow()) {
            ...
            if (result != DFB_OK) {
                DirectFBError("DirectFBGLScreenCursor::set: "
                              "Unable to get window size", result);
            }
            result = window->Resize(window, size.width(), size.height());
            if (result != DFB_OK) {
                DirectFBError("DirectFBGLScreenCursor::set: Unable to resize window", result);
            }

            IDirectFBSurface *windowSurface;
            result = window->GetSurface(window, &windowSurface);
            if (result != DFB_OK) {
                DirectFBError("DirectFBGLScreenCursor::set: Unable to get window surface", result);
            } else {
                result = windowSurface->Clear(windowSurface, 0, 0, 0, 0);
                if (result != DFB_OK) {
                    DirectFBError("DirectFBGLScreenCursor::set: Unable to clear surface", result);
                }

                result = windowSurface->Blit(windowSurface, surface, 0, 0, 0);
                if (result != DFB_OK) {
                    DirectFBError("DirectFBGLScreenCursor::set: Unable to blit to surface", result);
                }
            }
            result = windowSurface->Flip(windowSurface, 0, DSFLIP_NONE);
            if (result != DFB_OK) {
                DirectFBError("DirectFBGLScreenCursor::set: Unable to flip window", result);
            }
...
DirectFBGLScreen::DirectFBGLScreen(int display_id)
    : QGLScreen(display_id), d_ptr(new DirectFBGLScreenPrivate(this))
{
    DirectFBGLScreenPrivate::instance = this;
    setSurfaceFunctions(new DirectFBGLScreenSurfaceFunctions(this, displayId));
}

DirectFBGLScreen::~DirectFBGLScreen()
{
    if (DirectFBGLScreenPrivate::instance == this)
        DirectFBGLScreenPrivate::instance = 0;
    delete d_ptr;
}

DirectFBGLScreen *DirectFBGLScreen::instance()
{
    return DirectFBGLScreenPrivate::instance;
}

int DirectFBGLScreen::depth(DFBSurfacePixelFormat format)
{
    switch (format) {
    case DSPF_A1:
        return 1;
...
int DirectFBGLScreen::depth(QImage::Format format)
{
    int depth = 0;
    switch(format) {
...
void DirectFBGLScreenPrivate::setFlipFlags(const QStringList &args)
{
    ...
    if (index >= 0) {
        ...
        foreach(const QString &flip, flips) {
            if (flip == QLatin1String("wait"))
                ...
                qWarning("DirectFBGLScreen: Unknown flip argument: %s",
                         qPrintable(flip));
        }
    } else {
...
void DirectFBGLScreenPrivate::onWindowEvent(QWSWindow *window, QWSServer::WindowEvent event)
{
    if (event == QWSServer::Raise) {
        ...
        if (windowSurface && windowSurface->key() == QLatin1String("directfb")) {
             static_cast<DirectFBWindowSurface*>(windowSurface)->raise();
        }
    }
}

QPixmapData *DirectFBGLScreenPrivate::createPixmapData(QPixmapData::PixelType type) const
{
    return new DirectFBPixmapData(q, type);
}
...
static void printDirectFBInfo(IDirectFB *fb, IDirectFBSurface *primarySurface)
{
    ...
    qDebug("Device: %s (%s), Driver: %s v%i.%i (%s) Pixelformat: %d (%d)\n"
           ...
           DirectFBGLScreen::getImageFormat(primarySurface), dev.acceleration_mask,
...
bool DirectFBGLScreen::connect(const QString &displaySpec)
{
    qDebug("[DIRECTFBGL] Connect():connecting screen device\n");
    DFBResult result = DFB_OK;
    ...
    {   // pass command line arguments to DirectFB
        ...
        qDebug("[DIRECTFBGL] DirectFBInit...");
        result = DirectFBInit(&argc, &argv);
        if (result != DFB_OK) {
            DirectFBError("DirectFBGLScreen: error initializing DirectFB",
                          result);
        }
        delete[] argv;
    }
    ...
    qDebug("[DIRECTFBGL] DirectFBCreate...");
    result = DirectFBCreate(&d_ptr->dfb);
    if (result != DFB_OK) {
        DirectFBError("DirectFBGLScreen: error creating DirectFB interface",
                      result);
        return false;
    }
    ...
    if (displayArgs.contains(QLatin1String("systemonly"), Qt::CaseInsensitive)) {
        if (d_ptr->directFBFlags & VideoOnly) {
            qWarning("DirectFBGLScreen: error. videoonly and systemonly are mutually ADMINISTRATIVE");
        } else {
    ...
    DirectFBPaintEngine::initImageCache(imageCacheSize);
#endif
    ...
    QImage::Format pixelFormat = DirectFBGLScreen::getImageFormat(surface);
    ...
    case QImage::Format_RGB32:
        ...
        break;
    case QImage::Format_Indexed8:
        qWarning("DirectFBGLScreen::connect(). Qt/DirectFB does not work with the LUT8  pixelformat.");
        return false;
    case QImage::NImageFormats:
    case QImage::Format_Invalid:
    case QImage::Format_Mono:
    ...
    QScreen::d = DirectFBGLScreen::depth(pixelFormat);
    data = 0;
    lstep =  0;
    size = 0;
    ...
    if (result != DFB_OK) {
        DirectFBError("DirectFBGLScreen::connect: "
    ...
    if (!qws_size.isEmpty()) {
        ...
        if (!rx.exactMatch(qws_size)) {
            qWarning("DirectFBGLScreen::connect: Can't parse QWS_SIZE=\"%s\"", qPrintable(qws_size));
        } else {
            int *ints[2] = { &w, &h };
            for (int i=0; i<2; ++i) {
                *ints[i] = rx.cap(i + 1).toInt();
                if (*ints[i] <= 0) {
                    qWarning("DirectFBGLScreen::connect: %s is not a positive integer",
    ...
    if (w <= 0 || h <= 0) {
        ...
        if (d_ptr->dfbLayer->GetSurface(d_ptr->dfbLayer, &layerSurface) != DFB_OK) {
            ...
            qWarning("ReleaseSurface %x",layerSurface);
    ...
    qDebug("[DIRECTFBGL] Screen size:(width=%i,height=%i)",w,h);
    dw = w;
    dh = h;
    ...
    // Setting bkg color of the display layer
    if (backgroundColor.isValid()) {
        // Switch to administrative mode to allow setBackgroundMode/SetBackgroundColor
        ...
        if (result != DFB_OK) {
            DirectFBError("DirectFBGLScreenCursor::connect: "
                          "Unable to set background mode", result);
        }

        // Switch back to ADMINISTRATIVE mode where layer allow ADMINISTRATIVE access
        //!\ If EXCLSUIVE set, it remains traces when moving window
...
void DirectFBGLScreen::disconnect()
{
    qDebug("[DIRECTFBGL] Disconnect(): Releasing primary surface, layers and screen");
#if defined QT_DIRECTFB_IMAGEPROVIDER_KEEPALIVE
...
bool DirectFBGLScreen::initDevice()
{
    qDebug("[DIRECTFBGL] initDevice():Initializing device");

#ifndef QT_NO_DIRECTFB_MOUSE
    qDebug("[DIRECTFBGL] initDevice():Initializing mouse");
    if (qgetenv("QWS_MOUSE_PROTO").isEmpty()) {
        QWSServer::instance()->setDefaultMouse("None");
        d_ptr->mouse = new DirectFBMouseHandler;
    }
#endif
#ifndef QT_NO_DIRECTFB_KEYBOARD
    qDebug("[DIRECTFBGL] initDevice():Initializing keyboard");
    if (qgetenv("QWS_KEYBOARD").isEmpty()) {
        QWSServer::instance()->setDefaultKeyboard("None");
        d_ptr->keyboard = new DirectFBGLKeyboardHandler(QString());
    }
#endif

#ifdef QT_DIRECTFB_CURSOR
    qDebug("[DIRECTFBGL] initDevice():Initializing cursor");
    qt_screencursor = new DirectFBGLScreenCursor;
#elif !defined QT_NO_QWS_CURSOR
    qWarning("[DIRECTFBGL] initDevice():Initializing sofware cursor");
...
void DirectFBGLScreen::shutdownDevice()
{
    qDebug("[DIRECTFBGL] shutdownDevice():Shutting down device");
#ifndef QT_NO_DIRECTFB_MOUSE
    delete d_ptr->mouse;
    d_ptr->mouse = 0;
#endif
...
void DirectFBGLScreen::setMode(int width, int height, int depth)
{
   qDebug("[DIRECTFBGL] setMode: w:%i,h:%i,d:%i\n",width,height,depth);
    d_ptr->dfb->SetVideoMode(d_ptr->dfb, width, height, depth);
}

void DirectFBGLScreen::blank(bool on)
{
    qDebug("[DIRECTFBGL] Qblank\n");
    d_ptr->dfbScreen->SetPowerMode(d_ptr->dfbScreen,
                                   (on ? DSPM_ON : DSPM_SUSPEND));
}

QWSWindowSurface *DirectFBGLScreen::createSurface(QWidget *widget) const
{
        if (widget->inherits("QGLWidget")) {
            qDebug("[DIRECTFBGL] QGLWidget detected (winid:%x) => creating server side DirectFBGLWindowSurface", widget);
            return new DirectFBGLWindowSurface(d_ptr->flipFlags, const_cast<DirectFBGLScreen*>(this),qobject_cast<QGLWidget*>(widget));
        }
        return new DirectFBWindowSurface(d_ptr->flipFlags, const_cast<DirectFBGLScreen*>(this), widget);
}

QWSWindowSurface *DirectFBGLScreen::createSurface(const QString &key) const
{
    if (key == QLatin1String("directfb")) {
        qDebug("[DIRECTFBGL] creating client side DirectFBWindowSurface\n");
        return new DirectFBWindowSurface(d_ptr->flipFlags, const_cast<DirectFBGLScreen*>(this));
    }
    else if(key == QLatin1String("directfbgl"))
    {
        qDebug("[DIRECTFBGL] QGLWidget detected => creating client side DirectFBGLWindowSurface\n");
        return new DirectFBGLWindowSurface(d_ptr->flipFlags, const_cast<DirectFBGLScreen*>(this));
    }
    return QScreen::createSurface(key);
}


void DirectFBGLScreen::exposeRegion(QRegion r, int)
{
    Q_UNUSED(r);
}

void DirectFBGLScreen::solidFill(const QColor &color, const QRegion &region)
{
    Q_UNUSED(color);
    Q_UNUSED(region);
}
...
void DirectFBGLScreen::solidFill(IDirectFBSurface *surface, const QColor &color, const QRegion &region)
{
    if (region.isEmpty())
        return;
...
QImage::Format DirectFBGLScreen::alphaPixmapFormat() const
{
    return d_ptr->alphaPixmapFormat;
}

bool DirectFBGLScreen::initSurfaceDescriptionPixelFormat(DFBSurfaceDescription *description,
                                                        QImage::Format format)
{
    const DFBSurfacePixelFormat pixelformat = DirectFBGLScreen::getSurfacePixelFormat(format);
    if (pixelformat == DSPF_UNKNOWN)
        return false;
    description->flags |= DSDESC_PIXELFORMAT;
    description->pixelformat = pixelformat;
    if (DirectFBGLScreen::isPremultiplied(format)) {
...
uchar *DirectFBGLScreen::lockSurface(IDirectFBSurface *surface, DFBSurfaceLockFlags flags, int *bpl)
{
    void *mem;
    const DFBResult result = surface->Lock(surface, flags, &mem, bpl);
    if (result != DFB_OK) {
        DirectFBError("DirectFBGLScreen::lockSurface()", result);
    }
...
void DirectFBGLScreen::flipSurface(IDirectFBSurface *surface, DFBSurfaceFlipFlags flipFlags,
...
/**
 * Return the default native window for EGLContext
 */
bool DirectFBGLScreenSurfaceFunctions::createNativeWindow(QWidget *widget, EGLNativeWindowType *native)
{
    Q_UNUSED(widget);
    *native = (EGLNativeWindowType)0;
    return true;
}


#if defined QT_DIRECTFB_IMAGEPROVIDER_KEEPALIVE
void DirectFBGLScreen::setDirectFBImageProvider(IDirectFBImageProvider *provider)
{
    Q_ASSERT(provider);
    if (d_ptr->imageProvider)
        d_ptr->imageProvider->Release(d_ptr->imageProvider);
...
void DirectFBGLScreen::waitIdle()
{
    d_ptr->dfb->WaitIdle(d_ptr->dfb);
}

IDirectFBWindow *DirectFBGLScreen::windowForWidget(const QWidget *widget) const
{
    if (widget) {
        ...
        if (surface && surface->key() == QLatin1String("directfb")) {
            return static_cast<const DirectFBGLWindowSurface*>(surface)->directFBWindow();
        }
    }
    return 0;
}

IDirectFBSurface * DirectFBGLScreen::surfaceForWidget(const QWidget *widget, QRect *rect) const
{
    Q_ASSERT(widget);
    ...
    const QWSWindowSurface *surface = static_cast<const QWSWindowSurface*>(widget->windowSurface());
    if (surface && surface->key() == QLatin1String("directfb")) {
        return static_cast<const DirectFBGLWindowSurface*>(surface)->surfaceForWidget(widget, rect);
    }
    return 0;
}

#ifdef QT_DIRECTFB_SUBSURFACE
IDirectFBSurface *DirectFBGLScreen::subSurfaceForWidget(const QWidget *widget, const QRect &area) const
{
    Q_ASSERT(widget);
    QRect rect;
    ...
    if (surface) {
        ...
        if (!rect.isNull()) {
            ...
            if (result != DFB_OK) {
                DirectFBError("DirectFBGLScreen::subSurface(): Can't get sub surface", result);
            }
        }
    }
    return subSurface;
...
{
    return DirectFBGLScreen::instance() ? DirectFBGLScreen::instance()->surfaceForWidget(widget, rect) : 0;
}
#ifdef QT_DIRECTFB_SUBSURFACE
Q_GUI_EXPORT IDirectFBSurface *qt_directfb_subsurface_for_widget(const QWidget *widget, const QRect &area)
{
    return DirectFBGLScreen::instance() ? DirectFBGLScreen::instance()->subSurfaceForWidget(widget, area) : 0;
}
#endif
#ifdef QT_DIRECTFB_WM
Q_GUI_EXPORT IDirectFBWindow *qt_directfb_window_for_widget(const QWidget *widget)
{
    return DirectFBGLScreen::instance() ? DirectFBGLScreen::instance()->windowForWidget(widget) : 0;
}

#endif


#include "directfbglscreen.moc"
