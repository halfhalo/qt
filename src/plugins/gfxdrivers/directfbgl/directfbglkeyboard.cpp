...
class GLKeyMap : public QHash<DFBInputDeviceKeySymbol, Qt::Key>
{
public:
    GLKeyMap();
};

Q_GLOBAL_STATIC(GLKeyMap, keymap);

class DirectFBGLKeyboardHandlerPrivate : public QObject
{
    Q_OBJECT
public:
    DirectFBGLKeyboardHandlerPrivate(DirectFBGLKeyboardHandler *handler);
    ~DirectFBGLKeyboardHandlerPrivate();

    void suspend();
    void resume();

private:
    DirectFBGLKeyboardHandler *handler;
...
DirectFBGLKeyboardHandlerPrivate::DirectFBGLKeyboardHandlerPrivate(DirectFBGLKeyboardHandler *h)
      ...
      lastUnicode(0), lastKeycode(0), lastModifiers(0)
{
    ...
    if (!fb) {
        qCritical("DirectFBGLKeyboardHandler: DirectFB not initialized");
        return;
    }

    DFBResult result;
    result = fb->CreateInputEventBuffer(fb, DICAPS_KEYS, DFB_FALSE,
                                        &eventBuffer);
    if (result != DFB_OK) {
        DirectFBError("DirectFBGLKeyboardHandler: "
...
void DirectFBGLKeyboardHandlerPrivate::suspend()
{
    keyboardNotifier->setEnabled(false);
}

void DirectFBGLKeyboardHandlerPrivate::resume()
{
    eventBuffer->Reset(eventBuffer);
    keyboardNotifier->setEnabled(true);
}

DirectFBGLKeyboardHandlerPrivate::~DirectFBGLKeyboardHandlerPrivate()
{
    if (eventBuffer)
        eventBuffer->Release(eventBuffer);
}

void DirectFBGLKeyboardHandlerPrivate::readKeyboardData()
{
    if(!qt_screen)
        return;
    ...
    for (;;) {
        ...
        if (ret == -1) {
            if (errno != EAGAIN)
                qWarning("DirectFBGLKeyboardHandlerPrivate::readKeyboardData(): %s",
                         strerror(errno));
            return;
        }
            ...
            !(input.type & (DIET_KEYPRESS|DIET_KEYRELEASE)))
        {
            ...
            if (first) {
                qWarning("DirectFBGLKeyboardHandler - Getting unexpected non-keyboard related events");
                first = false;
            }
            break;
        }
...
DirectFBGLKeyboardHandler::DirectFBGLKeyboardHandler(const QString &device)
    : QWSKeyboardHandler()
{
    Q_UNUSED(device);
    d = new DirectFBGLKeyboardHandlerPrivate(this);
}

DirectFBGLKeyboardHandler::~DirectFBGLKeyboardHandler()
{
    delete d;
}

// Conflict with KeyMap (inot libQtGui!)
// Keys must match remocontrol.c into DirectFB-linux-x86/src/DirectFB-1.2/inputdrivers/ce3100_ir
GLKeyMap::GLKeyMap()
{
    ...
    /* REMOTE SPECIFIC */
    insert(DIKS_CHANNEL_UP            , Qt::Key_Plus);
    insert(DIKS_CHANNEL_DOWN          , Qt::Key_Minus);

}

QT_END_NAMESPACE
#include "directfbglkeyboard.moc"
